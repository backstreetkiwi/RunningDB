...
public class EditTrainingWindow extends Window {
    ...
    private void refreshTableRuns() {
        ...
        for (Run run : this.training.getBean().getRuns()) {
            ...
            pace = timeConverter.convertToPresentation(StatisticsUtil.getPace(run.getDistance(), run.getTime()), String.class, null);
        ...
        if(trainingDistance!=null && trainingTime!=null) {
            this.tableRuns.setColumnFooter("pace", timeConverter.convertToPresentation(StatisticsUtil.getPace(trainingDistance, trainingTime), String.class, null));
        }
